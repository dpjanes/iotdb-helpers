 ...
 *  d.selector - after the method has executed, this converts whatever
 *             it produces into what the result the called really wants.
 *             if not defined, it will end up being the enter 'self' result
 *  d.outputs - after everything is run, all the results (e.g. from the selector)
 ...
 *          })
 */
const series = _d => _.promise.denodeify((_self, done) => {
    const self = Object.assign({}, _self);
