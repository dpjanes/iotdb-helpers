...
/**
 */
const _async = (call, after) => Q.denodeify((self, done) => {
    done(null, self)
 ...
 *  rx integration
 */
const observe = _paramd => Q.denodeify((_self, done) => {
    const self = Object.assign({}, _self)
    ...
    const ops = d.inputs.map((input, index) => inner_done => {
        ...
        Q(Object.assign({}, next_self, input, counter))
            ...
            .then(sd => {
                ...
                if (d.delay && (index < d.inputs.length - 1)) {
                    if (d.verbose) {
            ...
            .catch(error => {
                if (d.error) {
                    inner_done(null, d.error(error, next_self) || null)
                } else {
                    inner_done(error)
                }
            })
    ...
    const _on_result = (error, outputs) => {
        ...
        if (d.output_flatten) {
            if (_.is.Function(d.output_flatten)) {
                next_self[d.outputs] = d.output_flatten(next_self[d.outputs])
            } else {
