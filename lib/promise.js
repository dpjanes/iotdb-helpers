...
const series = _d => Q.denodeify((_self, done) => {
    ...
    const ops = d.inputs
        .map(partial => Object.assign({}, self, partial))
        .map(nd => inner_done => {
            Q(nd)
                .then(d.method)
                .then(sd => {
 ...
 *
 *  XXX - this does not collect outputs yet
...
const page = _d => Q.denodeify((_self, done) => {
    ...
    const _run = pager => {
        ...
        Q(self)
            ...
            .then(series(d))
            .then(sd => {
                assert.ok(_.is.Array(sd[d.outputs]), `${method}: the outputs should always be an array`)
 ...
 */
const do_done = (actual_done, optional_self, _keys) => actual_self => {
    if (optional_self && _keys) { 
        const sd = Object.assign({}, optional_self)

        const keys = Array.isArray(_keys) ? _keys : _keys.split(",");
        keys.forEach(key => {
