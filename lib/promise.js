...
const _rebind = q => {
    ...
    q.validate = (...rest) => _rebind(q.then(validate(...rest)))
    ...
    q.end = (...rest) => _rebind(q.then(do_done(...rest)).except(...rest))
...
/**
 */
const validate = f => Q.denodeify((self, done) => {
    const _ = require("..");

    [ "required", "accepts" ].forEach((t_key, t_index) => {
        _.mapObject(f[t_key] || {}, (allowed, key) => {
            if (!_.is.Array(allowed)) {
                allowed = [ allowed ]
            }

            const value = self[key]
            if (t_index && _.is.Undefined(value)) {
                return
            }

            const ok = allowed.find(a => {
                if (_.is.Function(a) && a(value)) {
                    return true
                } else if (_.is.Dictionary(a)) {
                    let ok = true

                    _.mapObject(a, (a_f, a_key) => {
                        if (_.is.Function(a_f) && !a_f(value[a_key])) {
                            ok = false
                        } else {
                            assert.ok(false, `only functions checkers implemented sorry - work to be done`)
                        }
                    })
            ...
            if (ok) {
            } else if (_.is.Undefined(value)) {
                assert.ok(false, `${f.method || "[unknown]"}: ${key}: this value is required`)
            } else {
                assert.ok(false, `${f.method || "[unknown]"}: ${key}: this value is not the required type`)
            }
        })
    })
