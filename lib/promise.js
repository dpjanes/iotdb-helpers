...
// console.log("A.run", pager)
        ...
        Q(rolling_self)
            ...
            .then(sd => {
// console.log("B", sd.cursor)
// console.log("C", d.outputs ? true : false, d.outputs ? sd[d.outputs].length : null)
                if (d.outputs) {
...
const _rebind = q => {
    q.then = (...rest) => _rebind(q._then(...rest))
    ...
    q.end = (...rest) => _rebind(q.then(do_done(...rest)).catch(rest[0]))
...
/*
    ...
    [
        ...
        { key: "requires", required: true },
        { key: "accepts", required: false },
    ].forEach(t => {
        _.mapObject(f[t.key] || {}, (allowed, key) => {
            if (!_.is.Array(allowed)) {
                allowed = [ allowed ]
            }

            const value = self[key]
            if (!t.required && _.is.Nullish(value)) {
                return
            }

            if (t.required && _.is.Undefined(value)) {
                assert.ok(false, `${f.method}: ${key}: value is required`)
            }
            ...
            allowed.find(a => {
                if (_.is.Function(a)) {
                    if (!a(value)) {
                        assert.ok(false, `${f.method}: ${key}: value is not the required type (got: ${value})`)
                    }
                } else if (_.is.Dictionary(a)) {
                    _.mapObject(a, (a_f, a_key) => {
                        const a_value = value[a_key]

                        if (_.is.Nullish(a_value) && !t.required) {
                            return
                        }
                    
                        if (_.is.Function(a_f)) {
                            if (!a_f(a_value)) {
                                assert.ok(false, `${f.method}: ${key}/${a_key}: value is not the required type (got: ${a_value})`)
                                ok = false
                            }
                        } else {
                            assert.ok(false, `${f.method}: only functions checkers implemented sorry - work to be done`)
                        }
                    })

                    return true
                } else {
                    assert.ok(false, `${f.method}: only function or Dictionary allowed (got: ${a})`)
                }
            })
