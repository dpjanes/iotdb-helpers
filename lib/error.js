...
/**
 */
const otherwise = (test, update) => error => {
    const _ = require("..")
    ...
    if (_.is.Function(update)) {
        update(error.self, error)
        return error.self
    } else if (_.is.Dictionary(update)) {
        return Object.assign({}, error.self, update)
    } else {
        console.log("#", "_.error.otherwise: second argument should be Function or Dictionary")
        throw error
    }
}
